...
type WebSocketClient struct {
	...
	DeviceType   string             // 디바이스 타입 ("shinnara", "naradevice")
...
func (s *WebSocketServer) readPump(client *WebSocketClient) {
	...
	welcomeMessage, err := models.EncodeBinaryMessage(welcomeMsg, "shinnara")
	if err == nil {
		select {
		...
		default:
			...
			if binaryMsg, clientType, err := models.DecodeBinaryMessage(message); err == nil {
				...
				case models.BinaryMessageTypeConnect:
					...
					if s.onConnect != nil {
						modelClient := &models.Client{
							...
							UserCode:      userCode,
							Type:          config.ClientTypeShinnara, // WebSocket은 항상 shinnara
							UseBinaryMode: true,
						}
				...
				case models.BinaryMessageTypePing:
					...
					pongMessage, err := models.EncodeBinaryMessage(pongMsg, "shinnara")
					if err != nil {
