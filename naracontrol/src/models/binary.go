...
// Shinnara용 인증 생성: 첫3바이트 합산
func generateShinnaraAuth() uint32 {
	byte1 := randomByte()
	byte2 := randomByte()
	byte3 := randomByte()
...
// Shinnara 인증 검증
func validateShinnaraAuth(auth uint32) bool {
	byte1 := byte(auth >> 24)
	byte2 := byte(auth >> 16)
	byte3 := byte(auth >> 8)
	byte4 := byte(auth)
...
func CreateBinaryConnectMessage(companyCode, userCode, source string) []byte {
	...
	clientType := source
	if source != "infree" && source != "naradevice" {
		clientType = "infree" // 기본값
	}
