...
class _DashboardLineChartState extends State<DashboardLineChart> with TickerProviderStateMixin {
  ...
  Widget _buildCardLayout(BuildContext context) {
    ...
    final cards = widget.data.asMap().entries.map((entry) {
      ...
      return ChartDataCard(
        ...
        icon: dailyData.userCount > (index > 0 ? widget.data[index - 1].userCount : 0)
            ? Icons.trending_up
            : dailyData.userCount < (index > 0 ? widget.data[index - 1].userCount : 0)
            ? Icons.trending_down
            : Icons.trending_flat,
      );
    }).toList();
              ...
              touchTooltipData: LineTouchTooltipData(
                ...
                getTooltipItems: (List<LineBarSpot> touchedSpots) {
                  return touchedSpots.map((LineBarSpot touchedSpot) {
                    if (touchedSpot.x.toInt() >= 0 && touchedSpot.x.toInt() < widget.data.length) {
  ...
  List<FlSpot> _generateAnimatedSpots() {
    ...
    return List.generate(widget.data.length, (index) {
      final progress = (animationProgress * widget.data.length - index).clamp(0.0, 1.0);
