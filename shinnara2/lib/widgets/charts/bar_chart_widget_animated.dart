...
class _DashboardBarChartState extends State<DashboardBarChart> with TickerProviderStateMixin {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: Responsive.getResponsiveMargin(context) * 1.5), // 간격 1.5배 증가
  ...
  Widget _buildCardLayout(BuildContext context) {
    ...
    final total = widget.data.fold<double>(0, (sum, data) => sum + data.userCount);
        ...
        return BarChart(
          BarChartData(
            ...
            maxY: widget.data.isNotEmpty
                ? widget.data.map((e) => e.userCount).reduce((a, b) => a > b ? a : b).toDouble() *
                      1.2
                : 50,
            barTouchData: BarTouchData(
              ...
              touchTooltipData: BarTouchTooltipData(
                ...
                getTooltipItem: (group, groupIndex, rod, rodIndex) {
                  ...
                  final dataIndex = widget.data.indexWhere((data) => data.hour == hour);
                  final barProgress = (_animation.value * 1.5 - (dataIndex * 0.1)).clamp(0.0, 1.0);
                  final currentDisplayValue = (hourlyData.userCount * barProgress).round();
              ...
              leftTitles: AxisTitles(
                sideTitles: SideTitles(
                  ...
                  interval: widget.data.isNotEmpty
                      ? (widget.data.map((e) => e.userCount).reduce((a, b) => a > b ? a : b) / 5)
                            .roundToDouble()
                      : 10,
                  getTitlesWidget: (double value, TitleMeta meta) {
                    return SideTitleWidget(
  ...
  List<BarChartGroupData> _generateAnimatedBarGroups() {
    ...
    return List.generate(widget.data.length, (index) {
      ...
      final barProgress = (animationProgress * 1.5 - (index * 0.1)).clamp(0.0, 1.0);
      ...
      return BarChartGroupData(
        ...
        barRods: [
          BarChartRodData(
            toY: animatedValue,
            color: isTouched ? widget.barColor.withValues(alpha: 0.8) : widget.barColor,
            ...
            backDrawRodData: BackgroundBarChartRodData(
              show: true,
              toY: widget.data.map((e) => e.userCount).reduce((a, b) => a > b ? a : b).toDouble(),
      ...
      child: ClipRRect(
        ...
        child: DataTable(
          ...
          rows: widget.data.map((item) {
            final totalUsers = widget.data.fold<double>(0, (sum, data) => sum + data.userCount);
            final percentage = totalUsers > 0 ? (item.userCount / totalUsers * 100) : 0.0;
  ...
  Widget _buildMobileDataView(BuildContext context) {
    return SizedBox(
      height: 80,
        ...
        itemBuilder: (context, index) {
          ...
          return Container(
            ...
            child: Column(
              ...
              children: [
                Text(
                  '${item.hour.toString().padLeft(2, '0')}시',
                  style: TextStyle(
                    fontSize: 10,
                    fontWeight: FontWeight.bold,
