...
class _SeatPopupMenuState extends ConsumerState<SeatPopupMenu> {
      ...
      children: [
              ...
              Expanded(
                child: DropdownButton<int>(
                  ...
                  items: [1, 2, 3, 4, 5, 6, 8, 10, 12].map((hours) {
                    return DropdownMenuItem(value: hours, child: Text('$hours시간'));
                  }).toList(),
                  onChanged: (value) {
                    if (value != null) {
                      setState(() {
        ...
        if (isOccupied) ...[
          ...
          Row(
            children: [
              Expanded(
                child: OutlinedButton(onPressed: () => _extendTime(30), child: const Text('30분')),
              ),
              const SizedBox(width: 8),
              Expanded(
                child: OutlinedButton(onPressed: () => _extendTime(60), child: const Text('1시간')),
              ),
              const SizedBox(width: 8),
              Expanded(
                child: OutlinedButton(onPressed: () => _extendTime(120), child: const Text('2시간')),
              ),
            ],
          ),
        ],
  ...
  void _checkIn() {
    if (_userNameController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('이용자 이름을 입력해주세요')));
      return;
    }
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('좌석 ${widget.seat.number}에 체크인되었습니다')));
  }
  ...
  void _checkOut() {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('좌석 ${widget.seat.number}에서 체크아웃되었습니다')));
  }

  void _extendTime(int minutes) {
    ref.read(seatProvider.notifier).extendTime(widget.seat.id, minutes);
    Navigator.of(context).pop();
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$minutes분 연장되었습니다')));
  }
  ...
  void _changeStatus(SeatStatus status) {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('좌석 ${widget.seat.number} 상태가 변경되었습니다')));
  }
